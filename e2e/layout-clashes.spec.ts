...
test.describe('Home Page Layout Issues', () => {
  test('should display two-column layout on desktop without overlap', async ({ page }) => {
  ...
  test('should align hero heading with content below it', async ({ page }) => {
    ...
    const firstFeatureCard = page.locator('[data-testid*="feature"], .feature, section').nth(1);
  ...
  test('should be full-width single column on mobile (375px)', async ({ page }) => {
  ...
  test('should handle short viewport heights (820px) gracefully', async ({ page }) => {
    ...
    if (panelCount > 0) {
      for (let i = 0; i < Math.min(panelCount, 3); i++) {
        ...
        if (panelBox && textBox) {
          ...
          expect(textBox.x + textBox.width).toBeLessThanOrEqual(panelBox.x + panelBox.width);
        }
      }
    }
  });
  ...
  test('should align tax rate cards properly', async ({ page }) => {
    ...
    const taxCards = page.locator('[data-testid*="tax"], .tax-card, text=/Oneida|Madison|Herkimer/').filter({ visible: true });
