...
test.describe('Cart and Estimates', () => {
  ...
  test('should have functionality to return to calculators', async ({ page }) => {
    await page.goto('/cart');
    
    // Look for button/link back to calculators
    const continueBtn = page.locator('a, button').filter({ hasText: /continue|calculators|shopping/i });
...
test.describe('Estimate View', () => {
  test('should load estimate page with valid ID', async ({ page }) => {
    ...
    await page.goto('/estimate/test', { waitUntil: 'networkidle' }).catch(() => {});
  ...
  test('should display estimate information when available', async ({ page }) => {
    ...
    if (await firstCalc.isVisible()) {
      ...
      const saveBtn = page.locator('button, a').filter({ hasText: /save|add to cart|checkout/i });
