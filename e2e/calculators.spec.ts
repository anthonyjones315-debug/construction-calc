...
test.describe('Calculator System', () => {
  ...
  test('should display calculator categories', async ({ page }) => {
    ...
    const categories = page.locator('[data-testid*="category"], .category, [role="listitem"]');
  ...
  test('should navigate to a specific calculator', async ({ page }) => {
    ...
    const firstCalc = page.locator('a').filter({ hasText: /concrete|framing|roofing/i }).first();
  ...
  test('should display results after calculation', async ({ page }) => {
    ...
    if (await firstCalc.isVisible()) {
      ...
      if (await inputs.count() > 0) {
        ...
        const results = page.locator('[data-testid*="result"], .result, [role="region"]');
        // Results might appear automatically or after a button click
        const calculateBtn = page.locator('button').filter({ hasText: /calculate|compute/i });
