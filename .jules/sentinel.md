...
## 2025-05-15 - Unauthenticated Expensive AI API Endpoints
**Vulnerability:** The AI optimization endpoint (`/api/ai/optimize`) was publicly accessible, allowing anyone to exhaust Anthropic API credits and bypass rate limits via IP rotation.
**Learning:** Public endpoints that wrap paid LLM APIs are high-value targets for abuse. Rate limiting by IP provides some friction but isn't a substitute for authentication on internal tools.
**Prevention:** Always require a valid user session for endpoints that incur significant external API costs. Combine authentication with rate limiting for defense-in-depth.
