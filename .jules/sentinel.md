...
## 2025-05-15 - Unauthenticated and Unprotected Weather API
**Vulnerability:** The `/api/weather` endpoint was publicly accessible, leaked raw error messages, and lacked rate limiting, allowing for potential quota exhaustion and information disclosure.
**Learning:** Even "utility" endpoints that don't directly handle user data can be vectors for abuse if they consume paid external quotas (Google Maps Geocoding) or leak environment details through error stack traces.
**Prevention:** Always apply a baseline "defense in depth" to API routes: authentication checks, rate limiting (even if in-memory), and generic error responses for clients while logging details to Sentry.
