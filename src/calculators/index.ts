...
export function calcWireGauge(i: WireGaugeInputs): CalculationResult[] {
  ...
  const distance = clamp(sanitizeNum(i.distance, 0), 0, 100000);
  const voltage: 120 | 240 = i.voltage === 240 ? 240 : 120;
  ...
  const table =
    i.material === "aluminum"
      ? copperTable.map((r, idx) => ({
          ...r,
          amps: copperTable[Math.max(0, idx - 2)]?.amps ?? r.amps,
        }))
      : copperTable;

  const match = table.find((r) => r.amps >= amps) ?? table[table.length - 1];
  ...
  const vDrop = safeDiv(2 * K * amps * distance, cm);
  const vDropPct = safeDiv(vDrop, voltage) * 100;
  ...
  return [
    {
      ...
      unit: "",
      highlight: true,
      description: `NEC ampacity: ${match.amps}A`,
    },
    {
      label: "Voltage Drop",
      value: vDrop.toFixed(2),
      unit: "V",
      description: `${vDropPct.toFixed(1)}% — NEC max is 3%`,
    },
    {
      label: "Wire Length",
      value: (distance * 2).toFixed(0),
      unit: "ft",
