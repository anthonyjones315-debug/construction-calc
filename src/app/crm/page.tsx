    ...
    <section className="mx-auto max-w-7xl p-6 lg:p-8">
      <div className="mb-8 flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-[--color-ink-mid]">
            Manage client records, track job status, and pipeline stages in real-time.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button 
              ...
              className={`p-2 w-10 h-10 flex items-center justify-center rounded-lg transition ${viewMode === "list" ? "bg-white shadow-sm text-[--color-blue-brand]" : "text-slate-500 hover:text-slate-800"}`}
            >
              <List className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setViewMode("board")}
              className={`p-2 w-10 h-10 flex items-center justify-center rounded-lg transition ${viewMode === "board" ? "bg-white shadow-sm text-[--color-blue-brand]" : "text-slate-500 hover:text-slate-800"}`}
            >
      ...
      {loading ? (
        <div className="flex h-32 items-center justify-center rounded-2xl border border-[--color-border] bg-white">
          <p className="text-sm font-medium text-[--color-ink-mid]">Loading CRM pipeline...</p>
        </div>
      ) : contacts.length === 0 ? (
        ...
        <>
          {viewMode === "board" ? (
            <CrmKanbanBoard initialContacts={contacts} />
          ) : (
