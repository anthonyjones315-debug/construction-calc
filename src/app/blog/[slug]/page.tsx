      ...
      <div className="flex flex-col min-h-screen">
        ...
        <main id="main-content" className="flex-1 bg-[--color-bg]">
          <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
            ...
            <p className="text-lg text-[--color-ink-dim] mb-10 leading-relaxed">{post.description}</p>
                  ...
                  return (
                    <div key={i} className="overflow-x-auto my-4">
                      <table className="w-full text-sm border border-gray-200 rounded-xl overflow-hidden">
                        <tbody>
                          {rows.filter(r => !r.match(/^\|[-\s|]+\|$/)).map((row, j) => {
                            const cells = row.split('|').filter(c => c.trim())
                            return (
                              <tr key={j} className={j === 0 ? 'bg-[--color-surface-alt] font-semibold' : 'border-t border-gray-100'}>
                                {cells.map((cell, k) => (
            ...
            <div className="mt-12 p-6 bg-[--color-orange-soft] rounded-2xl border border-[--color-orange-brand]/20">
              <p className="text-sm font-semibold text-[--color-orange-brand] mb-1">Try the calculator</p>
              <p className="text-sm text-[--color-ink-mid] mb-3">Use our free {post.category.toLowerCase()} calculator for instant, accurate estimates.</p>
              <Link
                href="/"
