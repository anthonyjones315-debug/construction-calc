...
export async function GET(req: Request) {
  try {
    ...
    const rl = checkMemoryRateLimit("weather-api", session.user.id, 20, 60_000);
    if (!rl.ok) {
    ...
    // Re-throw Next.js internal errors to ensure proper build-time bail-out and prevent insecure caching.
    if (error && typeof error === "object" && "digest" in error && error.digest === "DYNAMIC_SERVER_URL") {
      throw error;
    }
