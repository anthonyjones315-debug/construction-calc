...
export async function GET(req: Request) {
  try {
    ...
    const rl = checkMemoryRateLimit("weather-api", session.user.id, 10, 60000);
    if (!rl.ok) {
    ...
    const parsed = weatherQuerySchema.safeParse(queryParams);
    ...
    const { lat: latParam, lng: lngParam, address, zip } = parsed.data;
    ...
    if (latParam !== undefined && lngParam !== undefined) {
      ...
      return NextResponse.json(
        { error: "Geocoding service unavailable or missing params" },
        { status: 400 }
      );
    }
