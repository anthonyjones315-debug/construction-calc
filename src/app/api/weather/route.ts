...
/** Handles Next.js build-time errors when auth/headers are called during static generation. */
function isPrerenderError(error: unknown): boolean {
  ...
  return (
    ...
    (err.message?.toLowerCase().includes("during prerendering") &&
      err.message?.toLowerCase().includes("headers()"))
  );
}
...
export async function GET(req: Request) {
  try {
    ...
    const rl = checkMemoryRateLimit("weather-api", ip, 50, 900_000); // 50 requests per 15 mins
    if (!rl.ok) {
