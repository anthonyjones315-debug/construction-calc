...
function buildEstimateEmailHtml(estimate: z.infer<typeof estimatePayloadSchema>): string {
  const rows = estimate.results
    .map(
      (r) =>
        `<tr><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0">${escapeHtml(r.label)}</td><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;text-align:right">${escapeHtml(r.value)} ${escapeHtml(r.unit)}</td></tr>`
    )
    .join("");
  const budget =
