...
export async function POST(req: NextRequest) {
  try {
    ...
    const contextRows = [
      ...
      technicalMessageValue
        ? `<p><strong>Technical Error:</strong> ${escapeHtml(technicalMessageValue, { nl2br: true })}</p>`
        : "",
  ...
  <div style="font-size:14px;line-height:1.6">${escapeHtml(message, { nl2br: true })}</div>
