...
function formatAsHtml(s: string): string {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const contextRows = [
      ...
      userFacingMessageValue
        ? `<p><strong>User-Facing Summary:</strong> ${formatAsHtml(userFacingMessageValue)}</p>`
        : "",
      technicalMessageValue
        ? `<p><strong>Technical Error:</strong> ${formatAsHtml(technicalMessageValue)}</p>`
        : "",
  ...
  <div style="font-size:14px;line-height:1.6">${formatAsHtml(message)}</div>
