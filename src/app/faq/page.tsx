...
export const metadata: Metadata = {
  ...
  description: 'Frequently asked questions about Build Calc Pro construction calculators.',
}
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <main id="main-content" className="flex-1 bg-[--color-bg]">
          <div className="max-w-2xl mx-auto px-4 sm:px-6 py-12">
            <h1 className="text-3xl font-display font-bold text-[--color-ink] mb-2">Frequently Asked Questions</h1>
            <p className="text-[--color-ink-dim] mb-10">Everything you need to know about Build Calc Pro.</p>

            <div className="space-y-4">
              {FAQ_ITEMS.map((item, i) => (
                <details key={i} className="group bg-white rounded-2xl border border-gray-200/80 shadow-sm overflow-hidden">
                  <summary className="flex items-center justify-between px-6 py-4 cursor-pointer list-none font-semibold text-[--color-ink] hover:text-[--color-orange-brand] transition-colors select-none">
                    {item.q}
                    <span className="text-[--color-ink-dim] group-open:rotate-45 transition-transform text-xl leading-none ml-4 shrink-0">+</span>
                  </summary>
