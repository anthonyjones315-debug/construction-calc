...
function formatTimestamp(date: Date): string {
  // Bolt Optimization: Cached Intl.DateTimeFormat prevents per-render instantiation overhead
            ...
            >
              <div>
                ...
                <p className="mt-0.5 text-xs text-[--color-ink-dim]">
                  Updated {getDateTimeFormatter().format(new Date(est.updated_at))}
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm font-mono font-bold text-[--color-ink]">
                  {est.total_cost != null
                    ? getNumberFormatter({ style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(est.total_cost)
                    : "Pending"}
                </p>
                <span
