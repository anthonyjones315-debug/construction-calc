...
import { HardHat, User, LogOut, ChevronDown, Bookmark } from 'lucide-react'
  ...
  return (
    <header className="sticky top-0 z-50 bg-[--color-nav-bg] border-b border-white/10 shadow-lg">
      <div className="max-w-screen-xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <Link
          href="/"
          className="flex items-center gap-2 text-white font-display font-bold text-xl tracking-wide hover:text-[--color-nav-active] transition-colors"
          aria-label="Build Calc Pro Home"
        >
          <HardHat className="w-6 h-6 text-[--color-orange-brand]" aria-hidden />
          <span>BUILD CALC PRO</span>
          ...
          {status === 'loading' ? (
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 text-sm text-[--color-nav-text] hover:text-white transition-colors"
