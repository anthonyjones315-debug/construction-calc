...
const AFFILIATE_ADS: Partial<
  Record<string, { label: string; cta: string; href: string }>
> = {
  sprayfoam: {
    label: "Spray Foam Insulation Kit",
    cta: "Shop on Amazon",
    href: "https://amzn.to/4cGL1qN",
  },
  flooring: {
    label: "Flooring & Underlayment",
    cta: "Shop on Amazon",
    href: "https://amzn.to/4br0GZ4",
  },
  roofPitch: {
    label: "Roof Pitch Gauge & Tools",
    cta: "Shop on Amazon",
    href: "https://amzn.to/411y17Z",
  },
  framing: {
    label: "Framing Nailers & Guns",
    cta: "Shop on Amazon",
    href: "https://amzn.to/3P4YsXS",
  },
  paint: {
    label: "Paint Sprayers & Supplies",
    cta: "Shop on Amazon",
    href: "https://amzn.to/4lsT3Wo",
  },
};
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-[10px] font-bold uppercase tracking-widest text-[--color-ink-dim]">
            Sponsored
          </p>
      ...
      <span className="shrink-0 text-xs font-bold text-[--color-orange-brand] group-hover:underline whitespace-nowrap">
        {ad.cta} →
      </span>
    </a>
  );
  ...
  return (
    <div className="flex flex-col gap-1">
      {label && (
        <label
          htmlFor={inputId}
          className="text-sm font-medium text-[--color-ink-mid]"
        >
          {label}
          {unit && (
...
export function ToggleGroup({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-sm font-medium text-[--color-ink-mid]">
        {label}
      </span>
      <div
        className="flex rounded-xl border border-gray-200 overflow-hidden bg-[--color-surface-alt]"
        role="group"
        aria-label={label}
      >
        {options.map((o) => (
...
export function WasteInput({
  ...
  const resolvedOnChange: (v: number) => void =
    onChange ?? onWasteChange ?? (() => undefined);
  const resolvedOnToggle: (v: boolean) => void =
    onToggle ?? onIncludeChange ?? (() => undefined);
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <span className="text-sm font-mono font-bold text-[--color-orange-brand] w-10 text-right">
          {resolvedValue}%
        </span>
      </div>
    </div>
...
export function Checkbox({ label, checked, onChange, id }: CheckboxProps) {
  ...
  return (
    <label
      htmlFor={inputId}
      className="flex items-center gap-2 text-sm text-[--color-ink-mid] cursor-pointer select-none"
    >
      <input
        id={inputId}
        type="checkbox"
    ...
    <div className="space-y-3">
      {label && (
        <p className="text-xs font-bold uppercase tracking-widest text-[--color-ink-dim]">
          {label}
          {unit && (
            <span className="font-normal normal-case text-[--color-ink-dim] ml-1">
              ({unit})
            </span>
          )}
        </p>
      ...
      {description && (
        <p className="text-xs text-[--color-ink-dim] -mt-1">{description}</p>
      )}
      {children}
    </div>
  );
