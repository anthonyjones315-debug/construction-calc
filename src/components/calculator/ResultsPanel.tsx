      ...
      {rest.length > 0 && (
        <div className="px-4 pb-4 grid grid-cols-2 gap-2">
          {rest.map((r, i) => (
            ...
            >
              <p className="text-[11px] text-[--color-ink-dim] font-medium uppercase tracking-wider">{r.label}</p>
              ...
              {r.description && <p className="text-[10px] text-[--color-ink-dim] mt-0.5">{r.description}</p>}
            </div>
          ))}
        </div>
      )}
