    ...
    try {
      ...
      if (res.ok) {
        const data = (await res.json()) as {
          prices: typeof MARKET_PRICES_BASE;
        };
    ...
    <div className="animate-fade-up space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[--color-ink-dim] mt-1">
            Build a material list with live pricing estimates
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-[--color-surface] rounded-2xl border border-gray-200/80 shadow-sm p-5">
          <h2 className="text-xs font-bold uppercase tracking-widest text-[--color-ink-dim] mb-4">
            Add Material
          </h2>
          <div className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                step={0.01}
                onChange={(e) =>
                  setMaterialQty(parseFloat(e.target.value) || 0)
                }
            ...
            {selectedMaterial && marketPrices[selectedMaterial] && (
              <p className="text-xs text-[--color-ink-dim]">
                Unit price: ${marketPrices[selectedMaterial].price.toFixed(2)} /{" "}
                {marketPrices[selectedMaterial].unit}
              </p>
            )}
            <button
        ...
        <div className="lg:col-span-2 bg-[--color-surface] rounded-2xl border border-gray-200/80 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-xs font-bold uppercase tracking-widest text-[--color-ink-dim]">
              Materials
            </h2>
          ...
          {budgetItems.length > 0 && (
            <div className="px-5 py-4 border-t border-gray-100 bg-[--color-surface-alt] flex items-center justify-between">
              <span className="text-sm font-bold text-[--color-ink] uppercase tracking-wide">
                Total Estimate
              </span>
      ...
      {budgetItems.length > 0 && <EstimateDisclaimer className="mx-0 mb-0" />}
