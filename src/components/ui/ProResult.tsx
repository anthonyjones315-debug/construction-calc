    ...
    <section className="flex min-h-[180px] flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/60 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {primary.label === "Total Yards" ? (
          <span className="text-[10px] text-[--color-nav-text]/70">
      ...
      <div className="flex flex-col gap-2">
        <div className="rounded-xl border border-[--color-orange-brand]/40 bg-black/40 p-3 shadow-[0_10px_22px_rgba(247,148,29,0.14)]">
          ...
          <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[--color-nav-text]/75">
            {primary.unit}
          </p>
        </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {secondary.map((result) => (
              ...
              >
                ...
                <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-widest text-[--color-nav-text]/75">
                  {result.unit}
                </p>
              </div>
            ))}
        ...
        {localTip ? (
          <div className="rounded-xl border border-[--color-orange-brand]/35 bg-[--color-orange-brand]/10 p-3">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.15em] text-orange-500">
              Pro Tip
            </h3>
            <p className="mt-1 text-xs text-[--color-nav-text]/92">{localTip}</p>
          </div>
        ) : null}

        <div className="rounded-xl border border-white/15 bg-black/25 p-3">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.15em] text-white/80">
              Material List
            </h3>
            {onCopyOrder ? (
              <button
                type="button"
                onClick={onCopyOrder}
                className="inline-flex min-h-7 items-center gap-1 rounded-lg border border-white/25 px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-[--color-nav-text] transition-all duration-200 hover:border-orange-400 hover:text-white active:scale-[0.98]"
              >
                Copy
              </button>
            ) : null}
