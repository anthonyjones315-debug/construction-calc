...
export function ProInput({
  ...
  const LabelOrLegend = hasSelect ? "legend" : "span";

  const containerProps = hasSelect
    ? { className: "flex flex-col gap-1" }
    : { className: "flex flex-col gap-1 cursor-pointer", htmlFor: fieldId };

  return (
    <Container {...containerProps}>
      <LabelOrLegend className="mb-1 block w-full text-xs font-semibold uppercase tracking-[0.12em] text-copy-secondary">
        ...
        {helpText ? (
          <span id={helpTextId} className="block mt-0.5 text-[10px] font-normal normal-case text-copy-tertiary lg:hidden">
            {helpText}
          </span>
        ) : null}
      </LabelOrLegend>
