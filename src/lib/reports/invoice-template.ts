    ...
    : payload.results
        .map(
          ...
          <tr>
            ...
            <td style="padding: 10px 12px; border-bottom: 1px solid #e5e7eb; text-align: right; color: #374151; font-size: 13px;">${escapeHtml(String(safeNumber(row.value)))}</td>
            <td style="padding: 10px 12px; border-bottom: 1px solid #e5e7eb; text-align: right; font-weight: 600; color: #111827; font-size: 13px;">${escapeHtml(String(safeNumber(row.value)))}</td>
          </tr>`,
        )
        .join("");
