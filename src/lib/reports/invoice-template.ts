...
/** Robust inline HTML escaping function */
...
/** URL Sanitization to prevent javascript: scheme/XSS payload injection */
...
export function generateInvoiceHtml(input: InvoiceTemplateInput): string {
  ...
  const signature = payload.signature as
    | { signatureDataUrl?: string | null; signedAt?: string | null; signerName?: string | null }
    | undefined;

  const sanitizedLogoUrl = escapeHtml(sanitizeUrl(contractorLogoUrl));
  const sanitizedSignatureDataUrl = escapeHtml(sanitizeUrl(signature?.signatureDataUrl));

  const materialList = Array.isArray(payload.material_list) ? payload.material_list : [];
  const escapedMaterials = materialList.map(item => escapeHtml(item)).join(", ");
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  <body>
    <div class="page">
          ...
          ${
            ...
            ${
              tax
                ? `
            <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #f3f4f6;">
              <span style="font-size: 13px; color: #6b7280;">${safeTaxLabel}</span>
      ...
      <div style="margin-top: 32px; display: flex; gap: 24px;">
        <div style="flex: 1;">
          ...
          <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 8px; background: #ffffff;">
            <img src="${sanitizedSignatureDataUrl}" alt="Signature" style="height: 48px; object-fit: contain;" />
          </div>
      ...
      <span style="display:none;color:#ea580c;">${escapedMaterials}</span>
    </div>
