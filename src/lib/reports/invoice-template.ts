...
export function generateInvoiceHtml(input: InvoiceTemplateInput): string {
  ...
  const materialsListHtml = Array.isArray(payload.material_list)
    ...
    <div class="page">
      ...
      <div style="margin-top: 32px; display: flex; gap: 24px;">
        <div style="flex: 1;">
          ...
          ${signature.signedAt ? `<p style="font-size: 10px; color: #9ca3af; margin-top: 4px;">Signed ${getDateTimeFormatter({ year: "numeric", month: "short", day: "numeric" }, "en-US").format(new Date(signature.signedAt))}</p>` : ""}
        </div>
        <div style="flex: 1;">
      ...
      <span style="display:none;color:#ea580c;">${materialsListHtml}</span>
      </main>
    </div>
