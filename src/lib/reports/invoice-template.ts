...
export function generateInvoiceHtml(input: InvoiceTemplateInput): string {
  ...
  const materialRows = materialList
    .map(
      (item) => `
    <div style="display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #f3f4f6;">
      <span style="font-size: 13px; color: #374151;">${escapeHtml(item)}</span>
    </div>`,
    )
    .join("");
  ...
  const safeSignerName = escapeHtml(signature?.signerName || "");
    ...
    <div class="page">
      ...
      ${
        materialList.length > 0
          ? `
      <div style="margin-top: 24px; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px;">
      ...
      <div style="margin-top: 32px; display: flex; gap: 24px;">
        <div style="flex: 1;">
          ...
          ${signature.signedAt ? `<p style="font-size: 10px; color: #9ca3af; margin-top: 4px;">Signed ${new Date(signature.signedAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })} by ${safeSignerName}</p>` : ""}
        </div>
        <div style="flex: 1;">
